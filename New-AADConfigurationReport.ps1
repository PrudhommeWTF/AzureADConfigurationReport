...
Param(
    ...
    [Alias('ATTON')]
    [Switch]$AppenTimestampToOutputName
)
...
if ([String]::IsNullOrWhiteSpace($ConfigFileBaseName)) {
    ...
    if ($AvailableConfigs.Count -gt 0) {
        ...
        if ($ConfigFileBaseName -eq $false) {
            ...
            #Tenant config created, loading configuration file from script folder
            try {
                $LoadedConfig = Import-Clixml -Path (Get-ChildItem -Filter '*.ini' | Where-Object -FilterScript {$_.BaseName -eq $ConfigFileBaseName}).FullName
            }
            catch {throw $_}
        }
    }
...
if ($AppenTimestampToOutputName) {
    $BaseOutputFileName = '{0}\{1}-{2:yyyyMMdd_HHmm}' -f $OutputFolder, $InitialDomain, $Start
} else {
    $BaseOutputFileName = '{0}\{1}' -f $OutputFolder, $InitialDomain
}

#Gathering reports indicator history (Xml files)
try {
    $IndicatorsHistory = Get-ChildItem -Path "$PSScriptRoot\Output\" -Filter "$InitialDomain-*.xml" -ErrorAction SilentlyContinue | Sort-Object -Property CreationTime -Descending | Select-Object -First 5 | ForEach-Object -Process {
...
#region Generating Output Files
...
} | Out-File -FilePath "$BaseOutputFileName.html" -Encoding 'utf8' -Force
...
$XmlData = @{
    ...
    RawData    = $Outputs | Where-Object -FilterScript {$_.ID -like 'CR*'}
} | ConvertTo-Xml -Depth 99 -NoTypeInformation
$XmlData.Save("$BaseOutputFileName.xml")
...
@{
    ...
    Indicators = $Indicators
    RawData    = $Outputs | Where-Object -FilterScript {$_.ID -like 'CR*'}
} | ConvertTo-Json -Depth 99 | Out-File -FilePath "$BaseOutputFileName.json" -Encoding utf8
#endregion Generating Output Files
...
Write-Output -InputObject "`tHTML: $BaseOutputFileName.html"
Write-Output -InputObject "`tXML : $BaseOutputFileName.xml"
Write-Output -InputObject "`tJSON: $BaseOutputFileName.json"
