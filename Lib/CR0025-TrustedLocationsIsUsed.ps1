...
$Output = @{
    ...
    Remediation            = 'Navigate to the following blade in the Azure AD Portal > Security > Named Locations. Configure the IP or Country locations following your business rules. Configure your Conditional Access Policies to use as inclusion or exclusion scope the configure Named Locations.<br>In general, Conditional Access policies may completely prevent users from authenticating to Azure Active Directory, and thorough testing is recommended. To avoid complete lockout, a ''Break Glass'' account with full Global Administrator rights is recommended in the event all other administrators are locked out of authenticating to Azure Active Directory. This ''Break Glass'' account should be excluded from Conditional Access Policies and should be configured with the longest pass phrase feasible. This account should only be used in the event of an emergency and complete administrator lockout.'
